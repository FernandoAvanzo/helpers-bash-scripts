...
EOF
  # Ensure /etc exists and a resolv.conf is present to silence nspawn warning
  install -D -m 0644 /etc/resolv.conf "${ROOTFS}/etc/resolv.conf" || true
...
install_userspace_in_container(){
  ...
  # Validate platform plugin & plugin registry
  nspawn_run "test -r /usr/lib/libcamhal/plugins/ipu6epmtl.so" || die "Missing /usr/lib/libcamhal/plugins/ipu6epmtl.so in container"
  nspawn_run "GST_PLUGIN_PATH=/usr/lib/x86_64-linux-gnu/gstreamer-1.0 gst-inspect-1.0 icamerasrc >/dev/null" || die "icamerasrc plugin not found in container"
}
...
# Verbose logs into journal for debugging
set -x
exec systemd-nspawn \
  --directory="${ROOTFS}" --machine="${MACHINE}" \
  --resolv-conf=replace-host --console=pipe \
  "${BINDS[@]}" \
  /bin/bash -lc "export LIBCAMHAL_PLUGIN_PATH=/usr/lib/libcamhal/plugins; \
                 export GST_PLUGIN_PATH=/usr/lib/x86_64-linux-gnu/gstreamer-1.0; \
                 exec gst-launch-1.0 -v icamerasrc device-name=${SENSOR} ! \
                   video/x-raw,format=NV12,width=${WIDTH},height=${HEIGHT},framerate=${FPS}/1 ! \
                   queue ! videoconvert ! video/x-raw,format=YUY2 ! \
                   v4l2sink device=${V4L2_NODE} sync=false qos=false"
WRAP
