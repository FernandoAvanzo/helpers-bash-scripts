...
# Motivo: o pacote notion-repackaged está desatualizado e pode congelar em tela branca.
#
...
# 3) cria um launcher de usuário em ~/.local/bin/notion-webapp;
# 4) cria um atalho .desktop em ~/.local/share/applications/notion-webapp.desktop.
#
# Navegadores compatíveis detectados automaticamente:
#   google-chrome-stable, google-chrome, chromium, chromium-browser,
#   brave-browser, microsoft-edge-stable
#
# Uso:
#   ./install_notion_resilient.sh
#   sudo ./install_notion_resilient.sh --purge-broken
#   ./install_notion_resilient.sh --browser google-chrome-stable
#   ./install_notion_resilient.sh --software-rendering
#   ./install_notion_resilient.sh --url https://www.notion.so

set -Eeuo pipefail
IFS=$'\n\t'

readonly SCRIPT_NAME="${0##*/}"
readonly APP_NAME="Notion Web App"
readonly DESKTOP_ID="notion-webapp.desktop"
...
usage() {
  cat <<'EOF'
Uso:
  ./install_notion_resilient.sh [opções]
  sudo ./install_notion_resilient.sh [opções]
...
Opções:
  ...
  --no-desktop              Não cria atalho .desktop
...
setup_target_user() {
  if is_root; then
    if [[ -n "${SUDO_USER:-}" && "${SUDO_USER}" != "root" ]]; then
      ...
      die "Ao executar como root, use sudo a partir do usuário que receberá o atalho (ex.: sudo ./${SCRIPT_NAME})."
    fi
  else
    TARGET_USER="${USER}"
    TARGET_HOME="${HOME}"
...
purge_old_packages_if_requested() {
  ...
  local -a installed=()
  if package_installed notion-app; then installed+=("notion-app"); fi
  if package_installed notion-app-enhanced; then installed+=("notion-app-enhanced"); fi
...
ensure_user_dirs() {
  ...
  install -d -m 0755 -o "${TARGET_UID}" -g "${TARGET_GID}" -- \
    "${bin_dir}" "${apps_dir}" "${config_dir}" "${profile_dir}"
}
...
if [[ -n "\${NOTION_WEBAPP_EXTRA_FLAGS}" ]]; then
  # Intencional: permitir múltiplas flags extras separadas por espaços.
...
ensure_local_bin_on_path_notice() {
  if [[ -x "${TARGET_HOME}/.local/bin/notion-webapp" ]]; then
    if ! sudo -u "${TARGET_USER}" -- bash -lc 'printf "%s" "$PATH"' | grep -qE '(^|:).*/\.local/bin(:|$)'; then
...
main() {
  ...
  cleanup_old_user_state
  ensure_user_dirs "${bin_dir}" "${apps_dir}" "${config_dir}" "${profile_dir}"
  ...
  ln -sfn "${wrapper_path}" "${compat_wrapper_path}"
  chown -h "${TARGET_UID}:${TARGET_GID}" "${compat_wrapper_path}"

  if [[ "${create_desktop}" == "1" ]]; then
    write_file_as_target_user \
      "${desktop_path}" "0644" \
      "$(build_desktop_file "${wrapper_path}" "${APP_NAME}")"
  fi
  ...
  if [[ "${create_desktop}" == "1" ]]; then
    log "Atalho criado em: ${desktop_path}"
  fi
