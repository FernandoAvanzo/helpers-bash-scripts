...
#
# What changed vs v55:
#  - We were writing frames to the wrong device (/dev/video42 turned out to be an
#    IPU6 capture node on your system, while browsers expect frames on a
#    v4l2loopback device). Now we enforce a single v4l2loopback at **/dev/video0**
#    and feed that.
#  - Safer pipeline for browser compatibility: NV12 -> videoconvert -> YUY2 -> v4l2sink.
#  - Keep nspawn fixes (console/DNS) and clean Noble PPA config.
...
ensure_v4l2loopback(){
  # Always reload to guarantee a single, predictable node at /dev/video0
...
EOF
  # Remove any stale jammy ipu6 lists to avoid conflicts
...
install_userspace_in_container(){
  ...
  nspawn_run "apt-get install -y --no-install-recommends ca-certificates gnupg curl gstreamer1.0-tools v4l-utils libcamhal0 gstreamer1.0-icamera"
  # Sanity
...
make_relay_service(){
  ...
  BINDS=()
  for p in /dev/video* /dev/media* /dev/v4l-subdev* /dev/mei*; do [ -e "$p" ] && BINDS+=( "--bind=$p" ); done
  printf "%s
" "${BINDS[@]}" > /etc/ipu6.bind-args
...
ExecStart=/bin/bash -lc '
  set -euo pipefail;
  mapfile -t BINDS < /etc/ipu6.bind-args || true;
  exec systemd-nspawn --directory="/var/lib/machines/ipu6-noble" --machine="ipu6-noble" \
...
main(){
  ...
  install_userspace_in_container
  make_relay_service
  log "Done. Check with: v4l2-ctl --list-devices (look for \"Intel MIPI Virtual Camera\" at ${V4L2_NODE})"
}
